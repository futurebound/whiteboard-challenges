...
class SLL {
  ...
  remove(offset) {
    ...
    if(offset === 1) { //if first node is the node to be deleted
      ...
      let current = this.head; //declaring temp var current aNode assigning value of head
      let counter = 0; //declaring counter to track offset
      while (counter < offset - 1) { //while the counter is less than the offset
        previous = current; //assign value of current node (initially head) to previous, then the value of next nodes iNodeex as while loop continues
        current = current.next; //assign value of currents next to current
        counter++; //increment counter for while loop stop coNodeition
      }
      ...
      this.length--; //decremeNode SLL length
